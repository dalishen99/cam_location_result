package com.hyf.iot.widget.webview
...
import com.hyf.iot.utils.PhoneUtils
import com.hyf.iot.widget.PageStateLayout
import java.util.ArrayList

class WebViewProxy(private val mAct: Activity, private val mWebView: WebView,
                   private val mPageLayout: PageStateLayout? = null,
                   private val mWebActionCallBack: WebActionCallBack? = null) {

    companion object {
        const val TAG = "WebViewProxy"
    }
    ...
    private fun initPageLayout() {
        mPageLayout?.setOnPageErrorClickListener {
            if (!TextUtils.isEmpty(mUrl)) {
                isError = false
                mPageLayout.setPage(PageStateLayout.PageState.STATE_LOADING)
    ...
    private fun initWebViewClient() {
        mWebView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView, url: String) {
                ...
                if (!isError && mPageLayout != null && mPageLayout.getPageState() !== PageStateLayout.PageState.STATE_ERROR) {
                    if (!redirect) {
                        mHandler?.postDelayed({
                            mPageLayout.setPage(PageStateLayout.PageState.STATE_SUCCEED)
                        }, 100)
                    }
                }
    ...
    private fun registerJSInterface() {
        mWebView.addJavascriptInterface(JsObject(), Constant.WEB.SCHEME)
    }
